...
app.get("/",(request,response)=>{
    c.query('select * from studdetails',(err,result,field)=> {
        if (err) throw err;
...
app.post("/employee_view/:id",(request,response)=>{
    ...
    c.query('select * from studdetails where id=?',[id],(err,result,field)=> {
        if (err) throw err;
        response.send(result);
    });
...
app.post("/employee",(request,response)=>{
    ...
    c.query("INSERT INTO studdetails ( firstname, lastname, location, email, dob, education) values (?,?,?,?,?,?)",[f_name2,l_name2,location2,email2,dob2,education2],function(error,result){
...
app.post("/employee_update",(request,response)=>{
    ...
    c.query("update studdetails set firstname=?,lastname=?,location=?,email=?,dob=?,education=? where id=?",[firstName,lastName,location,email,dob,education,id],function(error,result){
...
app.post("/employee_delete/:id",(request,response)=>{
    const {id} = request.params;
    c.query('delete from studdetails where id=?',[id],(err,result,field)=> {
        if (err) throw err;
        response.send(result);
    });
